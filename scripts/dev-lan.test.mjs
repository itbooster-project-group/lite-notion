...
test('LAN runner process config passes exact LAN env without API bind override', () => {
  ...
  assert.equal(config.gatewayOrigin, 'http://192.168.1.42:8080');
  ...
  assert.equal(api.env.CORS_ORIGIN, 'http://192.168.1.42:8080');
  ...
  assert.equal(web.env.NEXT_PUBLIC_API_BASE_URL, 'http://192.168.1.42:8080');
  assert.equal(web.env.NEXT_PUBLIC_API_MOCKING, 'disabled');
  assert.equal(web.env.NEXT_PUBLIC_COLLABORATION_URL, 'ws://192.168.1.42:8080/collaboration');
  assert.deepEqual(collaboration.args, ['--filter', '@lite-notion/collaboration', 'dev']);
  assert.equal(collaboration.env.COLLABORATION_ALLOWED_ORIGIN, 'http://192.168.1.42:8080');
