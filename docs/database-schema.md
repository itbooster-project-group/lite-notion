...
Задача управления проектами. В MVP у задачи один владелец, он же исполнитель: назначить задачу другому пользователю нельзя. Отдельная таблица исполнителей не проектируется — делиться в MVP не с кем, поскольку нет ни workspace, ни команд, ни шеринга страниц. Многопользовательская работа с задачами появится вместе с workspace отдельным изменением.

Связь с `PAGE` необязательная: задача может быть личной и не относиться ни к какому документу. Привязка допустима только к странице того же владельца.
...
| `owner_id` | `uuid` FK | Владелец задачи, он же исполнитель |

#### Чем `TASK` отличается от блока типа `todo`

Обе сущности дают отметку о выполнении, поэтому границу стоит проговорить явно.

| | Блок типа `todo` | `TASK` |
| --- | --- | --- |
| Где живёт | внутри страницы, в потоке содержимого | самостоятельная сущность |
| Состояние | бинарное: отмечен или нет | три статуса |
| Срок | нет | `due_date` |
| Где виден | только на своей странице | в сквозном списке задач с фильтрами |

Синхронизации между ними нет: отметка блока `todo` не создаёт и не изменяет задачу.
...
- **Nullable `Task.page_id`.** Задача может быть личной и не относиться ни к какому документу, поэтому привязка к странице остаётся необязательной.
- **Один владелец у задачи.** В MVP нет workspace, команд и шеринга страниц, поэтому назначать задачу некому: junction-таблица исполнителей не проектируется, владелец задачи является её исполнителем. Возврат к нескольким исполнителям придёт вместе с workspace, где всё равно потребуется модель прав.
