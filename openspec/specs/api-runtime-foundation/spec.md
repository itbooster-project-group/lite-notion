...
API MUST при запуске валидировать `NODE_ENV`, `PORT` и `CORS_ORIGIN`, преобразовывать порт в число и не начинать принимать HTTP-запросы при невалидной конфигурации. Допустимыми значениями `NODE_ENV` MUST быть `development`, `test` и `production`; `PORT` MUST быть целым числом от 1 до 65535; `CORS_ORIGIN` MUST быть HTTP(S) origin без path, query и fragment.

#### Scenario: Запуск с конфигурацией по умолчанию
- **WHEN** `NODE_ENV`, `PORT` и `CORS_ORIGIN` не заданы
- **THEN** API запускается с `NODE_ENV=development`, портом `3001` и разрешённым origin `http://localhost:3000`
