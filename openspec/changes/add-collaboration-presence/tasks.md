...
- [x] 2.3 Реализовать безопасный mapper Awareness states -> validated presence users: malformed/empty states игнорируются, users дедуплицируются по `state.user.id`, clientID не используется как participant identity; representative выбирается по минимальному numeric clientID
...
- [x] 3.4 Реализовать полный idempotent cleanup только при page switch, editor/session unmount, logout/session loss или explicit destroy: unsubscribe listeners, очистить subscriptions/snapshots, destroy editor/provider и `Y.Doc`; сохранить Strict Mode safety
...
- [x] 4.2 Передать `PresenceUser` в `CollaborationCaret`; только `CollaborationCaret` публикует `Awareness state.user`, cursor и selection, без ручных ProseMirror decorations, отдельного Y.Map или presence lifecycle в JSX
...
- [x] 6.1 Расширить существующий `page-editor-collaboration` Playwright harness сценарием A с двумя разными authenticated users: оба видны в participants UI без reload
...
- [x] 6.4 В сценариях A/B проверить disconnect/reconnect и исчезновение пользователя после закрытия второго context согласно Awareness timeout semantics
