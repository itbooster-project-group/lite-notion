...
- Подключить глобальную типизированную конфигурацию окружения с обязательными `NODE_ENV`, `PORT` и `CORS_ORIGIN`, безопасной ошибкой запуска и шаблоном `.env` для локальной разработки.
...
- **Frontend:** код не меняется; шаблон локальной конфигурации разрешает origin `http://localhost:3000`.
