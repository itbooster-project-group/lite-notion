...
Persisted Yjs/derived JSON не считается trusted только потому, что текущие UI forms валидируют insertion. Перед static/public rendering работает отдельная normalization boundary, переиспользующая entity validators для `src`, `videoId`, `alignment`, `widthPercent`, `nodeId` и link `href`. `renderPageDocumentToHTML({ content, schemaVersion })` сначала явно допускает только поддерживаемую schema version и выбрасывает typed error для неизвестной version: удаление неизвестных future nodes schema v2 normalizer-ом v1 не является допустимой compatibility strategy. Это единственный public render/normalization entry point entity; schema-v1 normalizer остаётся internal implementation detail и не экспортируется через FSD barrel, поэтому version guard нельзя случайно обойти. Invalid link mark снимается с сохранением текста; invalid external media fail-safe исключается из renderable document. HTTP(S) links открываются в новой вкладке с `noopener noreferrer`, тогда как normalized `mailto:` сохраняет `href` без навязанного нового browser tab. Interactive schema output дополнительно не создаёт external element из invalid media attrs. Полная publication admission/migration policy остаётся future publication/Hocuspocus change и не создаёт второй mutable store.
...
Surface отдельно владеет только document-specific transient UI. При смене `doc` или editor instance lifecycle закрывает link/image/video/YouTube dialogs, сбрасывает bubble/slash state и coordinates, очищает RelativePosition/bookmark и handler refs. Global UI вне editor document не сбрасывается. Identity editor фиксируется callback-ом TipTap `onCreate` вместе с захваченным `Y.Doc`; сравнение только текущих props и временно возвращённого `useEditor` instance недостаточно. Пока identity созданного editor не совпадает с текущим doc, весь interactive subtree (`EditorContent`, toolbar, reorder, slash/bubble и forms) заменяется доступным `aria-busy` transition state. Поэтому stale editor не показывает content и не принимает command даже при быстрой замене A → B → C.

Link selection хранится как Yjs RelativePosition текущего binding и разрешается перед command; mismatch/unresolvable position не создаёт transaction. Официальный TipTap `editor.utils.createMappablePosition()` версии 3.30.5 рассмотрен, но не заменяет этот adapter: его public update contract требует map через каждую transaction, не предоставляет эквивалентную one-shot fail-safe resolution и внутри collaboration helper сводит неразрешимую Yjs position к `0`. Кроме того, текущие внутренние anchors сохраняют affinity диапазона при конкурентной вставке ровно на границе. До эквивалентной публичной гарантии custom adapter остаётся локальным implementation detail feature.

Bubble и slash menus используют единый fixed-position hook: ProseMirror selection/transaction, capture-scroll (включая container scroll), window resize и ResizeObserver планируют пересчёт через один `requestAnimationFrame`; несколько событий кадра не вызывают повторные synchronous layout reads/state updates. Flip/shift ограничивает popup viewport, cleanup удаляет listeners/observers и отменяет pending frame. Rich-text paragraphs/headings/lists/task/media presentation собрана в CSS module, тогда как TSX оставляет component layout и interactive positioning.
