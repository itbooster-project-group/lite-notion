...
- Follow-up: production collaboration, Hocuspocus persistence lifecycle, public route и asset uploads рассматриваются отдельными reviewed changes. До production/private documents требуется отдельно оценить asset proxy/controlled storage либо более строгий allowlist: текущие external media намеренно выполняют direct browser requests к произвольным HTTPS origins, и `no-referrer` не скрывает IP/network request.
