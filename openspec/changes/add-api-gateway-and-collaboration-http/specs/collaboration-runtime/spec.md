...
**Reason**: Центральное содержание требования — что открытое соединение сохраняет выданный режим до переподключения, переживая и отзыв прав, и истечение access-токена, — было зафиксировано как временное ограничение с оговоркой, что обе границы снимаются вместе. Этот change их снимает: сохранять требование, чьё главное утверждение стало ложным, нельзя.

**Migration**: Заменено требованием «Открытое соединение переавторизуется по сроку жизни токена», которое сохраняет прежние гарантии для новых соединений и добавляет периодическую переавторизацию открытых. Поведение при подключении не меняется; изменяется срок, в течение которого уже открытое соединение может работать с устаревшим решением, — теперь он ограничен сроком жизни access-токена.
