...
- Preserve the current bootstrap contract: `NODE_ENV`, `PORT`, `CORS_ORIGIN`, `DATABASE_URL`, `DATABASE_CONNECTION_TIMEOUT_MS`, `JWT_SECRET`, `INTERNAL_SERVICE_TOKEN`, `ACCESS_TOKEN_TTL_S`, `REFRESH_TOKEN_TTL_S`, and `BCRYPT_ROUNDS` are all required and validated; `apps/api/.env.example` provides the recommended local values. Configuration errors must never print the value of `JWT_SECRET` or `INTERNAL_SERVICE_TOKEN`.
