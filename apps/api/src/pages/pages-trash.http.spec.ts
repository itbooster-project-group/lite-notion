...
describe('pages trash HTTP contract', () => {
  ...
  describe('удаление', () => {
    ...
    it('убирает страницу из дерева и показывает её в корзине', async () => {
      ...
      await request(server()).delete(`/api/v1/pages/${root.id}`).set(authorization).expect(204);
    ...
    it('не публикует источник удаления наружу', async () => {
      const page = await createPage();
      await request(server()).delete(`/api/v1/pages/${page.id}`).set(authorization).expect(204);
  ...
  describe('восстановление', () => {
    ...
    it('поднимает страницу в корень, когда её родитель остался в корзине', async () => {
      const root = await createPage();
      const child = await createPage(root.id, 'child');
      await request(server()).delete(`/api/v1/pages/${child.id}`).set(authorization).expect(204);
      await request(server()).delete(`/api/v1/pages/${root.id}`).set(authorization).expect(204);
  ...
  describe('изоляция по владельцу', () => {
    ...
    it('не отвечает 403 ни на одном маршруте корзины', async () => {
      ...
      const statuses = await Promise.all([
        request(server()).delete(`/api/v1/pages/${page.id}`).set(foreignAuthorization),
        request(server()).post(`/api/v1/pages/${page.id}/restore`).set(foreignAuthorization),
      ]);
