...
/**
 * Актор вправе видеть страницу, но его роли не хватает для операции. Единственный
 * случай, когда маршруты страниц отвечают `403`: скрывать существование страницы,
 * которую он и так читает, не от кого.
 *
 * Отдельный класс, а не флаг в `PageNotFoundError`: бросить его можно только после
 * успешной проверки доступа, и правило «403 лишь для видимой страницы» держится
 * этим, а не внимательностью в контроллере.
 */
export class PageRoleInsufficientError extends Error {
  constructor() {
    super('Insufficient role for this operation on the page');
    this.name = 'PageRoleInsufficientError';
  }
}
