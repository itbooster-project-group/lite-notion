...
export class PasswordService implements OnModuleInit {
  ...
  /**
   * Дубль DTO-проверки для вызовов мимо HTTP: хеш от обрезанного пароля подошёл бы и
   * к полному паролю, и к его первым 72 байтам. `async` — чтобы это было отклонением
   * промиса, а не синхронным throw мимо `.catch()`.
   */
