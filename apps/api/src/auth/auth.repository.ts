...
/**
 * Доступ модуля аутентификации к базе. Репозиторий модульный, а не на сущность,
 * потому что регистрация пишет в `User` и `Session` одной транзакцией — такая
 * операция не принадлежит ни одной из таблиц по отдельности.
 *
 * Абстракция вынесена ради тестируемости: транзакции и `SELECT ... FOR UPDATE`
 * живут только здесь, и держать эту специфику в одном файле дешевле, чем
 * поднимать базу в unit-тестах сервисов.
 */
@Injectable()
